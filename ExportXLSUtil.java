...
public class ExportXLSUtil 
{
    ...
    */
	 ...
	 * @return
	 */
	public static HSSFWorkbook exportExcelForCost(String title, String[] headers, List<Object[]> dataset, Boolean[] amounts) 
	{
		if(headers==null || headers.length==0)
		{
			return null;
		...
		if(dataset==null || dataset.isEmpty())
		{
			...
			sheet.createFreezePane(0,1,0,1);
			//填充表格头
			//fillCostHead(sheet,headers,headStyle);
			return workbook;
		}
		HSSFCellStyle dataStyle=getDataStyle(workbook);// 生成单元格数据样式
		HSSFCellStyle moneyStyle=getMoneyStyle(workbook);// 生成金额单元格数据样式
		...
		for(int i=0;i<sheetNum;i++)
		{
			...
			//fillCostHead(sheet,headers,headStyle);
			//填充表格数据
			//fillCostData(sheet,dataset,fIndex,lIndex,amounts,dataStyle,moneyStyle);
		}
		return workbook;
	}
	/**************************************************************/
	 ...
	 * @return
	 */
	public static HSSFWorkbook exportExcelForCostDetail(String title, String[] headers, List<Object[]> dataset, Boolean[] amounts) 
	{
		if(headers==null || headers.length==0)
		{
			return null;
		...
		if(dataset==null || dataset.isEmpty())
		{
			...
			sheet.createFreezePane(0,1,0,1);
			//填充表格头
			//fillCostDetailHead(sheet,headers,headStyle);
			return workbook;
		}
		HSSFCellStyle dataStyle=getDataStyle(workbook);// 生成单元格数据样式
		HSSFCellStyle moneyStyle=getMoneyStyle(workbook);// 生成金额单元格数据样式
		...
		for(int i=0;i<sheetNum;i++)
		{
			...
			//fillCostDetailHead(sheet,headers,headStyle);
			//填充表格数据
			//fillCostDetailData(sheet,dataset,fIndex,lIndex,amounts,dataStyle,moneyStyle);
		}
		return workbook;
	}
}